...
pred FMUnionSemantics[config:set Name] {
	...
	optional[Fp, Fm, config]
	optional[Fp, Fn, config]
	excludes[Fk, Fm, config]
	excludes[Fk, Fn, config]
	requires[Fm, Fn, config]

	--formulas
	(Fk in config) or (Fm + Fn in config)
}
...
pred FMIntersectionSemantics[config:set Name] {
	config not in FMIntersection.features
}
